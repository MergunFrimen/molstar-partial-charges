    ...
    ctor: class implements PluginBehavior<undefined> {
        ...
        private provider = {
            label: (loci: Loci) => {
                ...
                const charge = this.ctx.managers.interactivity.props.granularity === 'residue'
                    ? residueToCharge.get(typeId)!.get(id)
                    : atomIdToCharge.get(typeId)!.get(id);
                // TODO: change label for water based on entity.type
                const label = this.ctx.managers.interactivity.props.granularity === 'residue'
                    ? 'Residue charge'
                    : 'Atom charge';

                return `<b>${label}: ${charge ? charge?.toFixed(3) : 'undefined'}`;
            },
